...
local function HopServer()
    while hopserver do
        task.wait(3) -- Đợi một khoảng thời gian trước khi thử lại
        local servers = ReplicatedStorage:FindFirstChild("__ServerBrowser")
        if servers then
            -- Gọi lại Remote để lấy danh sách các server
            local availableServers = ReplicatedStorage.__ServerBrowser:InvokeServer()
            for serverId, serverInfo in pairs(availableServers) do
                -- Kiểm tra nếu server khác và có số người chơi ít
                if serverId ~= game.JobId and serverInfo["PlayerCount"] < 10 then
                    -- Thực hiện nhảy qua server
                    print("Đang nhảy sang server khác: " .. serverId)
                    ReplicatedStorage.__ServerBrowser:InvokeServer("teleport", serverId)
                    return -- Kết thúc sau khi nhảy sang server mới
                end
            end
        else
            warn("Không thể tìm thấy __ServerBrowser trong ReplicatedStorage.")
        end
    end
end

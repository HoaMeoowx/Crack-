...
local resetTime = 20 -- Thời gian reset nhân vật
...
function HopServer(maxPlayers)
    ...
    local function HandleTeleportError()
        ...
        local errorPrompt = CoreGui:FindFirstChild("ErrorPrompt", true) -- Tìm phần tử ErrorPrompt

        if errorPrompt then
            if errorPrompt.Visible and errorPrompt:FindFirstChild("TitleFrame") then
                local errorTitle = errorPrompt.TitleFrame:FindFirstChild("ErrorTitle")
            ...
            -- Kết nối sự kiện thay đổi thuộc tính Visible
